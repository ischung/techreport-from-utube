...
### #CI-1 [CI] 저장소 초기화 + Monorepo 스캐폴딩 + lint/format + pre-commit
...
### #CI-2 [CI] GitHub Actions 기본 워크플로 (lint + unit test)
...
### #CI-3 [CI] PR 보호 규칙 + 상태 배지 + KANBAN_TOKEN runtime-guard
...
### #CI-4 [CD] Dockerfiles × 2 + docker-compose.yml + 환경변수 설계
...
### #CI-5 [CD] main 머지 → 이미지 빌드 + compose up 스테이징 자동 배포
...
### #CI-6 [CD] 스테이징 배포 후 smoke test 자동 수행
...
### #CI-7 [Skeleton] `/api/health` + React `AppShell` "Hello Dashboard"
...
### #CI-8 [Skeleton] Config 로딩 + `.env.example` + LLM Provider 선택 스텁
...
### #CI-9 [Skeleton] Playwright E2E 1건 — "Dashboard 로드 + health 표시"
...
### #CI-16 [Security] SAST + 의존성 취약점 스캔
...
#### 배경
보안 게이트를 늦게 추가하면 축적된 취약점이 폭탄이 된다. CI 기반이 마련된 직후(#CI-2) 바로 SAST + 의존성 스캔을 붙여 P0로 둔다. `priority:p0 + mandatory-gate` 조합으로 PR 머지 차단.
...
### #CI-10 [Slice] US-01 키워드 입력 → 최근 1개월 영상 5개 표시 (Happy Path)
...
**연관 US**: US-01
**예상 소요**: 2일
**Depends on**: #CI-8, #CI-6
**Required by**: #CI-11, #CI-13
...
### #CI-11 [Slice] US-02 영상 선택 → 한국어 기술보고서 생성·저장 (Happy Path)
...
### #CI-12 [Slice] 분석 진행 UI (AnalyzingView + LogStream + Progress)
...
### #CI-13 [Slice] US-01 엣지 케이스 — 검색 0건 / 네트워크 오류
...
### #CI-14 [Slice] US-02 엣지 케이스 — 자막 없음 / LLM 타임아웃
...
### #CI-18 [QA] E2E 시나리오 3건 확장 (성공 / 자막 없음 / 검색 0건)
...
### #CI-19 [A11y] 다크 모드 WCAG AA 대비율 검토 + 키보드 네비 개선
...
### #CI-15 [CD] 프로덕션 배포 파이프라인 (Fly.io + Vercel) + 수동 승인
...
**Phase**: 3 운영화
**예상 소요**: 1일
**Depends on**: #CI-12, #CI-6
**Required by**: #CI-17, #CI-20
...
### #CI-17 [Ops] 에러 로깅 + Claude 토큰 누적 카운터 (`/api/health` 노출)
...
### #CI-20 [Docs] README 실행 가이드 + Provider 교체 실습 섹션 + 릴리즈 노트
